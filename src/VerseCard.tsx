...
interface Props { verse: Verse; index: number; selectedPosition?: number; highlightVerbs: boolean; showArabicVerses: boolean; showWordMeanings: boolean; language: 'both' | 'bn' | 'en'; onSelect: (key: string, word: Word) => void }

export default memo(function VerseCard({ verse, index, selectedPosition, highlightVerbs, showArabicVerses, showWordMeanings, language, onSelect }: Props) {
  return <article id={`ayah-${verse.key}`} className={!!selectedPosition ? 'verse active-verse' : 'verse'}>
    ...
    {showArabicVerses && <div className="arabic-verse" lang="ar" dir="rtl">{verse.parts.map((part, i) => <span key={i}>{part.word ? <button lang="ar" className={`arabic-word ${highlightVerbs && verse.words[part.word - 1]?.pos === 'V' ? 'is-verb' : ''} ${selectedPosition === part.word ? 'is-selected' : ''}`} onClick={() => onSelect(verse.key, verse.words[part.word! - 1])} aria-label={`Study ${part.text}, word ${part.word}`}>{part.text}</button> : <span className="pause-mark">{part.text}</span>}{i < verse.parts.length - 1 ? ' ' : ''}</span>)}</div>}

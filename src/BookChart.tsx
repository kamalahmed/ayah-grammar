...
export default function BookChart({ verb, showMeanings = true }: { verb: BookVerb; showMeanings?: boolean }) {
  ...
  return <section className="book-chart-section" aria-label={`Book conjugation ${verb.entry_number}`}>
    <div className="book-source-line"><span>BOOK {verb.level} <span aria-hidden="true">/</span> VERB {verb.entry_number}</span><a href={bookPageUrl(verb)} target="_blank" rel="noreferrer">Original page {verb.source.pdf_page} <ExternalLink size={13} /></a></div>
        ...
        <tbody>{bookPersons.map(person => <tr key={person}>
          ...
          {aspects.map(a => {
            ...
            return <td key={a.id} className={cell?.review_note_bn ? 'book-cell review-cell' : 'book-cell'}>
              {cell ? <><span className="conjugated-arabic" lang="ar" dir="rtl">{cell.ar}</span>{showMeanings ? <span className="book-verb-meaning" lang="bn">{cell.bn}{cell.correction_note_bn && <sup title={cell.correction_note_bn}>*</sup>}</span> : <span className="book-hidden-meaning" aria-label="Bangla meaning hidden">•••</span>}{cell.review_note_bn && <a className="book-review-link" href={bookPageUrl(verb, cell.source_page)} target="_blank" rel="noreferrer" title={cell.review_note_bn}>Check printed form ↗</a>}</> : <span className="empty-cell" aria-label={applicable ? 'Not supplied in this book' : 'Not applicable'}>{applicable ? '—' : 'n/a'}</span>}
            </td>
          })}
        </tr>)}</tbody>
      </table>
    ...
    <details className="book-source-details"><summary>Source & transcription notes</summary><p>Extracted from your local Level {verb.level} PDF, pages {verb.source.pages.join(', ')}. Entry numbers and table completeness were checked, with targeted visual checks of the text. The entire collection has not been independently proofread. Use the original page when a form or meaning looks unusual.</p>{corrections && <ul>{verb.conjugations.filter(c => c.bn_source).map(c => <li key={`${c.aspect}:${c.person}`} lang="bn">বইয়ে: {c.bn_source} → {c.bn}</li>)}</ul>}</details>
  </section>
}
